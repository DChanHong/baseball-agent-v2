...
from app.domains.baseball.tool.search_ticketing_guide.schemas import (
    ...
    SearchTicketingGuideToolResult,
)
...
class SearchTicketingGuideToolHandler:
    ...
    async def execute(
        self,
        tool_input: SearchTicketingGuideToolInput,
    ) -> SearchTicketingGuideToolResult:
        """사용자 질문을 embedding한 뒤 예매 안내 chunk만 검색합니다."""

        logger.info(
            "search_ticketing_guide tool started stadium_id=%s team_id=%s top_k=%d",
        ...
        limitations = [
            "ticket_inventory_not_supported",
            "ticketing_policy_may_be_outdated",
            "official_ticketing_source_should_be_checked",
        ]
        if not items:
            limitations.append("no_relevant_ticketing_guide_found")

        logger.info(
            "search_ticketing_guide tool completed stadium_id=%s count=%d answerable=%s",
            tool_input.stadium_id,
            len(items),
            bool(items),
        )

        return SearchTicketingGuideToolResult(
            stadium_id=tool_input.stadium_id,
