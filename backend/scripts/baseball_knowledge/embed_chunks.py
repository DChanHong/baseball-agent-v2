...
# 야구 지식 chunk JSONL을 OpenAI embedding으로 변환해 공용 pgvector 테이블에 upsert한다.
import argparse
import asyncio
import json
import os
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Embed baseball knowledge chunks and upsert them into local Supabase pgvector tables."
    )
    parser.add_argument(
        "--chunks",
        type=Path,
...
def validate_chunk(chunk: dict[str, Any], line_number: int) -> None:
    ...
    if chunk["stadium_id"] is not None:
        raise ValueError("Baseball knowledge chunks must have stadium_id=null")

    if chunk["team_id"] is not None:
        raise ValueError("Baseball knowledge chunks must have team_id=null")
