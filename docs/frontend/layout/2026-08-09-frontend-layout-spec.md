...
- [확인됨] 현재 MVP에는 실제 대화 목록 API, 팀/구장 네비게이션, 계정 메뉴, 설정 페이지가 구현되어 있지 않다.
...
- [확인됨] UI 개선 1차 범위의 좌측 사이드바는 구현됐다. 단, 세션 데이터는 아직 mock 목록이다.
...
- [확인 필요] mock 세션을 실제 conversation summary API와 연결해야 한다.
