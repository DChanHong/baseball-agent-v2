...
- 서울시 시설/주차 공식 페이지에서 구장 규모, 좌석 수, 수용인원, 주차규모, 주차요금을 확보했다.
- LG 공식 티켓 페이지에서 좌석 등급, 예매 기간, 수수료, 예매 한도, 취소, 관람 유의사항을 확보했다.
- 두산은 NOL 스포츠 두산베어스 페이지에서 잠실야구장 경기/예매 데이터는 확보했지만 좌석/요금 상세는 추가 검수 필요.

추가 보강 후보:

```text
두산 좌석/요금 공식 이미지 수동 검수
홈팀별 원정석/응원석 안내
잠실야구장 지하철 출구/버스 노선 공식 출처
화장실, 수유실, 팬샵, 게이트 위치 공식 출처
```
