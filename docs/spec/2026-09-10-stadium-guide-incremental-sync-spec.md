...
1. [x] 후보 목록·상세 diff 명령
2. [x] 승인·반려 명령
3. [x] 변경분 임베딩
4. [x] 로컬 revision 활성화
5. [x] 고척 전용 평가와 공통 회귀 평가

### Phase 4. Blog 6 작성과 구현 결과 반영

1. [x] 이 spec을 기반으로 Blog 6 초안 생성
2. [x] 구현 전 문제와 설계 결정을 먼저 기록
3. [x] 실제 구현 결과, 실패와 평가 수치를 반영해 글 완성
4. [x] 운영 schema 불일치와 복구 과정을 반영

### Phase 5. 운영 승격

1. [x] 사용자 승인 후 운영 migration 적용
2. [x] 평가를 통과한 고척·공통 revision 4건 승격
3. [x] 운영 DB의 활성 revision과 chunk 확인
4. [x] 운영 Tool 검색 검증
5. [x] 멱등 재실행 확인
6. [x] transaction rollback 방식의 이전 revision 복구 검증
...
- [x] pending 후보의 문서 유형 범위와 문장별 근거 검수
...
- [x] 운영 migration과 첫 revision 승격: 2026-09-11 실행
